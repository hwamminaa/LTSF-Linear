...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='recipe5_run_to_failure_1.csv',
...
class Dataset_Pred(Dataset):
    def __init__(self, root_path, flag='pred', size=None,
                 features='S', data_path='recipe1_run_to_failure_2.csv',
    ...
    def __read_data__(self):
        ...
        tmp_stamp['Time'] = pd.to_Timetime(tmp_stamp.Time)
        pred_Times = pd.Time_range(tmp_stamp.Time.values[-1], periods=self.pred_len + 1, freq=self.freq)

        df_stamp = pd.DataFrame(columns=['Time'])
        df_stamp.Time = list(tmp_stamp.Time.values) + list(pred_Times[1:])
        self.future_Times = list(pred_Times[1:])
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp.Time.apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp.Time.apply(lambda row: row.day, 1)
            df_stamp['weekday'] = df_stamp.Time.apply(lambda row: row.weekday(), 1)
            ...
            data_stamp = time_features(pd.to_Timetime(df_stamp['Time'].values), freq=self.freq)
            data_stamp = data_stamp.transpose(1, 0)
